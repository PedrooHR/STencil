...
module BlockALU #(
    ...
    input wire [SHIFTWIDTH*`PRECISION-1:0] S_BACK_TDATA,
    input wire [(SHIFTWIDTH+2)*`PRECISION-1:0] S_CENTER_TDATA,
    input wire [SHIFTWIDTH*`PRECISION-1:0] S_FRONT_TDATA,
  ...
  generate
    for (genvar I = 0; I < SHIFTWIDTH; I = I + 1) begin : gen_ALU_I
      ALU #(
          ...
          .S_AXIS_A(S_CENTER_TDATA[(I+0)*`PRECISION+:`PRECISION]),  // LEFT
          .S_AXIS_B(S_CENTER_TDATA[(I+1)*`PRECISION+:`PRECISION]),  // CENTER
          .S_AXIS_C(S_CENTER_TDATA[(I+2)*`PRECISION+:`PRECISION]),  // RIGHT
          .S_AXIS_D(S_UPPER_TDATA[I*`PRECISION+:`PRECISION]),       // UP
          .S_AXIS_E(S_BOTTOM_TDATA[I*`PRECISION+:`PRECISION]),      // BOTTOM
          .S_AXIS_F(S_BACK_TDATA[I*`PRECISION+:`PRECISION]),        // BACK
          .S_AXIS_G(S_FRONT_TDATA[I*`PRECISION+:`PRECISION]),       // FRONT
