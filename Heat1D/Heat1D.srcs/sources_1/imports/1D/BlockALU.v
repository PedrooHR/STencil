...
module BlockALU #(
    ...
    input wire [(SHIFTWIDTH+2)*`PRECISION-1:0] S_TDATA,
    input wire S_TVALID,
    input wire S_TLAST,
  ...
  generate
    for (genvar I = 0; I < SHIFTWIDTH; I = I + 1) begin : gen_ALU_I
      ALU #(
          ...
          .S_AXIS_A(S_TDATA[(I+0)*`PRECISION+:`PRECISION]),
          .S_AXIS_B(S_TDATA[(I+1)*`PRECISION+:`PRECISION]),
          .S_AXIS_C(S_TDATA[(I+2)*`PRECISION+:`PRECISION]),
